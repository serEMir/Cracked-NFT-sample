...
import {FlipMoodNFT} from "../../script/interactions/FlipMoodNFT.s.sol";
import {Test, console} from "forge-std/Test.sol";

contract MoodNftTest is Test {
    MoodNFT public moodNft;
    FlipMoodNFT public moodFlipper;
    ...
    function testFlipNftRevertsIfNotOwner() public {
        vm.prank(USER);
        moodNft.mintNft();
    ...
    function testFlipNft() public {
        vm.prank(USER);
        moodNft.mintNft();

        vm.prank(USER);
        moodNft.flipMood(0);
        assert(moodNft.getCurrentMood(0) == MoodNFT.Mood.SAD);
    }
}